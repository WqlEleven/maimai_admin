  ...
  <div class="container">
    <el-form :model="form" :rules="rules" ref="form" label-width="100px">
      ...
      <!-- <el-form-item prop="sell_date" label="预售日期" v-if="sell_show">
        <el-date-picker v-model="form.sell_date" type="date" placeholder="选择日期"></el-date-picker>
      </el-form-item> -->
      <el-form-item prop="delivery_date" label="发货日期" v-if="sell_show">
        <el-date-picker v-model="form.delivery_date" type="daterange" range-separator="至" 
		start-placeholder="开始日期" end-placeholder="结束日期" @blur ="changeDate" value-format="yyyy-MM-dd"></el-date-picker>
      </el-form-item>
      ...
      <el-form-item label>
        <el-row>
          <el-button type="primary" @click="handleAdd()" icon="el-icon-check" round>添加商品</el-button>
...
export default {
  ...
  methods: {
    ...
    handleColorPictureSuccess(res, file, fileList) {
      // console.log(res)
      //console.log(this.form.specifications)
      if (res.code == 0) {
        this.form.color[res.data.params.index].picture = res.data.file;
        // console.log(this.form.color)
        //this.form.specifications.push({});
        this.form.tmp = res.data.file;
      }
    },
    ...
    handleAdd() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.over == 0) {
            this.$axios
              .post("/admin/goods_add", this.$qs.stringify(this.form))
              .then(res => {
                // console.log(res);
                if (res.data.code == -1) {
